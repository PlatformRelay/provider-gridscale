...
import (
	...
	otherv1alpha1 "github.com/PlatformRelay/provider-gridscale/apis/namespaced/gridscale/v1alpha1"
...
func TestConnect_RejectsWrongManagedType(t *testing.T) {
	...
	_, err := c.Connect(context.Background(), &otherv1alpha1.PublicNetwork{})
	if err == nil {
		t.Fatal("Connect must reject a managed resource of the wrong type")
	}
	if !strings.Contains(err.Error(), errNotBackupList) {
		t.Errorf("error = %q, want it to contain %q", err, errNotBackupList)
	}
}
...
func TestConnect_PropagatesSetupError(t *testing.T) {
	...
	_, err := c.Connect(context.Background(), &v1alpha1.BackupList{})
	if err == nil {
...
func TestConnect_BuildsClientFromProviderSetup(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		json.NewEncoder(w).Encode(backupListResponse{}) //nolint:errcheck
	}))
	defer srv.Close()
	...
	ext, err := c.Connect(context.Background(), &v1alpha1.BackupList{})
	if err != nil {
		t.Fatalf("Connect: %v", err)
	}
	if _, err := ext.Observe(context.Background(), &v1alpha1.BackupList{}); err != nil {
...
func TestObserve_RejectsWrongManagedType(t *testing.T) {
	...
	_, err := ext.Observe(context.Background(), &otherv1alpha1.PublicNetwork{})
	if err == nil {
...
func TestObserve_SurfacesAPIError(t *testing.T) {
	...
	_, err := ext.Observe(context.Background(), &v1alpha1.BackupList{})
	if err == nil {
		t.Fatal("Observe must surface a non-2xx API response as an error")
	}
	if !strings.Contains(err.Error(), errList) {
		t.Errorf("error = %q, want it to contain %q", err, errList)
	}
}

func TestDelete_IsNoOp(t *testing.T) {
	ext := &external{}
	if _, err := ext.Delete(context.Background(), &v1alpha1.BackupList{}); err != nil {
...
func TestSetupGated_RegistersGVKAndDefersSetup(t *testing.T) {
	...
	want := v1alpha1.BackupList_GroupVersionKind
