#!/usr/bin/env bash
set -euo pipefail
CLUSTER=$(find examples-generated/cluster/observe -name '*.yaml' 2>/dev/null | wc -l | tr -d ' ')
NAMESPACED=$(find examples-generated/namespaced/observe -name '*.yaml' 2>/dev/null | wc -l | tr -d ' ')
